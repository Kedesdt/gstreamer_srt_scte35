...
void srt_thread(struct Config* config) {
    ...
    if (pthread_create(&thread_id, NULL, init_srt_client, config) != 0) {
...
void* init_srt_client(void* args) {

    struct Config* config = (struct Config*)args;
    //init_srt(config);
    //g_print("Endereco config: %d", config);
    srt_client_main(config);
...
int srt_client_main(struct Config* config) {
    ...
    sink = gst_element_factory_make("wasapisink", "audioSink");
    ...
    g_object_set(G_OBJECT(source), "keep-listening", TRUE, NULL);
    g_object_set(G_OBJECT(source), "uri", uri, NULL);
    g_object_set(G_OBJECT(source), "mode", 2, NULL);
