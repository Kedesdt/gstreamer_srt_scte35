...
#define PIPELINE_STR "videotestsrc is-live=True ! video/x-raw,framerate=30/1 ! x264enc tune=zerolatency ! queue ! mpegtsmux name=mux scte-35-pid=123 scte-35-null-interval=4500000000 ! srtsink uri=srt://:8888/"
...
send_splice(GstElement* mux, gboolean out)
{
    ...
    if (out)
    {
        ...
        g_print("%d", sit->splice_command_type);
        //sit->splice_command_type = 5;
    }
    else
