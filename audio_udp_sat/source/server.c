...
typedef void (*funcao)(struct Config *);

void udp_server(struct Config* config) {
    ...
    if (pthread_create(&thread_id, NULL, init_udp_server, config) != 0) {
        g_print("Erro ao criar thread");
    }
    else {
        g_print("Init_srt chamado\n");
    }
}
void * init_udp_server(void* args) {

    struct Config* config = (struct Config*)args;
    //init_srt(config);
    g_print("Endereco config: %d\n", config);
    udp_main(config);

    return NULL;
}

int init_udp(struct Config * config) {
    ...
    *config->stop = 0;
    ...
    g_print("%d, %d, %d, %d, %d\n", config->com, config->uri, config->device, config->running, config->stop);
    g_print("%d, %d\n", *config->running, *config->stop);
...
int udp_main(struct Config * config) {
    ...
    rtpPay        = gst_element_factory_make("rtpmp2tpay",      "rtpPay"       );
    ...
    if (!pipeline || !audioSource || !audioFilter || !audioFilter2 || !volume || !audioConvert || !audioResample || !audioEncoder || !mp3Parse || !audioQueue
        || !mp3Parse || !rtpPay || !tsparse || !muxer || !sink)
    {
    ...
    g_object_set(G_OBJECT(sink), "host", uri, NULL);
    g_print("Host configurado: %s\n", uri);
    free(uri);
    
    g_print("Porta UDP configurada: %d\n", config->port);
    g_object_set(G_OBJECT(sink), "port", config->port, NULL);
    g_print("ID configurado: %d\n", config->id);
    g_object_set(G_OBJECT(tsparse), "pcr-pid", config->id, NULL);
    


    //g_object_set(G_OBJECT(audioSource), "device", "\{0.0.1.00000000\}.\{eb8a86c3-c59a-4129-ac6a-fa4886311551\}", NULL);
    //\{0.0.1.00000000\}.\{d59148b0 - e4e6 - 4fd1 - 85f5 - 5eff3051bee9\}
    //g_object_set(G_OBJECT(audioSource), "device", "\{0.0.1.00000000\}.\{d59148b0-e4e6-4fd1-85f5-5eff3051bee9\}", NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline), audioSource, audioFilter, audioFilter2, volume, audioResample, audioEncoder, audioConvert, mp3Parse, audioQueue,
        muxer, rtpPay, tsparse, sink, NULL);
    ...
    g_print("audioConvert,   audioResample: %d\n", gst_element_link(audioConvert,  audioResample));
    g_print("audioResample,  audioFilter2 : %d\n", gst_element_link(audioResample, audioFilter2 ));
    g_print("audioFilter2,   volume       : %d\n", gst_element_link(audioFilter2,  volume       ));
    g_print("volume,         audioEncoder : %d\n", gst_element_link(volume,        audioEncoder ));
    ...
    g_print("muxer,          rtpPay       : %d\n", gst_element_link(muxer,         rtpPay       ));
    g_print("rtpPay,         tsparsev     : %d\n", gst_element_link(rtpPay,        tsparse      ));
    g_print("tsparse,        sink         : %d\n", gst_element_link(tsparse,       sink         ));
