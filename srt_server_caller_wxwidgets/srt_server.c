...
void srt_server(struct Config* config) {
...
void* init_srt_server(void* args) {
...
int init_srt(struct Config* config) {
...
int srt_main(struct Config* config) {
    ...
    GstElement* pipeline,       * audioSource,  * audioFilter,  * audioFilter2, * audioConvert, 
              * audioResample,  * audioEncoder, * mp3Parse,     * audioQueue,
              * muxer,          * sink;
    GstCaps* caps;
    GstBus* bus;
    GstMessage* msg;
    ...
    if (*config->mode) {
        g_object_set(G_OBJECT(sink), "mode", 1, NULL);
        //g_object_set(G_OBJECT(sink), "mode", GST_SRT_CONNECTION_MODE_CALLER, NULL);
        g_print("MODE CALLER\n");
    }
